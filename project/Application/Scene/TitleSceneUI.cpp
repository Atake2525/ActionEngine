...
void TitleSceneUI::Initialize(SpriteFactory& spriteFactory, Input& input, WinApp& winApp, Audio& audio) {
    // STARTとEXITのUIスプライトの初期化
    
    m_pInput = &input;
    m_pAudio = &audio;

    m_pressAnyKeySprite = spriteFactory.Create("Resources/Sprite/UI/press_any_key.png");
    ...
    Vector2 windowSize = { winApp.GetWindowSize() };
    ...
    std::shared_ptr<UI::Element> m_startButton;
    m_startButton = std::make_shared<UI::Button>();
    m_startButton->SetContext(spriteFactory);
    ...
    std::shared_ptr<UI::Element> m_exitButton;
    m_exitButton = std::make_shared<UI::Button>();
    m_exitButton->SetContext(spriteFactory);
    ...
    m_selectionGroup->Add(m_startButton);
    m_selectionGroup->Add(m_exitButton);
